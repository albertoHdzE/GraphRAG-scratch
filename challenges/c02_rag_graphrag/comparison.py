...
class MetrologyEngine:
    ...
    def generate_rich_metrics(self, results: Dict) -> Dict:
        ...
        # Path C Metrics (GraphRAG)
        graphrag_block = None
        if "graphrag" in results:
            graphrag_summary = results["graphrag"]["summary"]
            graphrag_trace_id = results["graphrag"]["trace_id"]
            graphrag_tokens = self.count_tokens(graphrag_summary)
            graphrag_faithfulness = self.calculate_faithfulness_score(
                chunks, graphrag_summary, trace_id=graphrag_trace_id
            )
            graphrag_density = self.calculate_information_density(
                chunks, graphrag_summary, trace_id=graphrag_trace_id
            )

            gmd = results["graphrag"].get("metadata", {}) or {}
            graphrag_block = {
                "latency_ms": results["graphrag"]["latency"],
                "total_tokens": graphrag_tokens,
                "info_density": graphrag_density,
                "faithfulness": graphrag_faithfulness,
                ...
                "graph_nodes_count": int(gmd.get("graph_nodes_count", 0)),
                "graph_edges_count": int(gmd.get("graph_edges_count", 0)),
                "efficiency": graphrag_tokens / (results["graphrag"]["latency"] / 1000),
            }
        ...
        if graphrag_block is not None:
            report["graphrag"] = graphrag_block
...
class ExpertInterpreter:
    ...
    def interpret_results(self, metrics: Dict, results: Dict, trace_id: Optional[str] = None) -> str:
        ...
        graphrag_metrics = metrics.get("graphrag")
        
        graphrag_section = ""
        if graphrag_metrics:
            graphrag_section = f"""
        System C (GraphRAG):
        - Info Density: {graphrag_metrics['info_density']:.2f}
        - Faithfulness: {graphrag_metrics['faithfulness']:.1f}/10
        - Efficiency: {graphrag_metrics['efficiency']:.1f} t/s
        - Graph Coverage: {graphrag_metrics['graph_coverage']:.2f}
        - Path Coherence: {graphrag_metrics['path_coherence']:.2f}
        - Traversal Depth: {graphrag_metrics['traversal_depth']}
            """

        prompt = f"""
        As a Legal AI Expert, analyze this retrieval-augmented comparison:
        ...
        Path C: {(results.get('graphrag', {}) or {}).get('summary', '')[:500]}...
        
        Task: Provide a deep interpretation of these results. Explain:
        1. Which path preserves more legal information and why?
        2. Is the RAU clustering mathematically sound based on the silhouette score?
        3. If GraphRAG is present, does graph coverage + path coherence suggest good multi-hop retrieval?
        4. A final recommendation on which synthesis is more reliable for identifying subtle contract risks.
